...
// The BlogLogFunc type is an adapter to allow the use of ordinary
// function as BlogLog mutator.
type BlogLogFunc func(context.Context, *ent.BlogLogMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f BlogLogFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.BlogLogMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.BlogLogMutation", m)
	}
	return f(ctx, mv)
}
